		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">教材信息</div>
						<div class="text "  >{{detail.jiaocaixinxi}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						...
						<el-button class="reserveBtn" 
							v-if="btnAuth('kechengxinxi','报名课程')" 
							@click="onAcross('cankexinxi','','','[1]','已报名请勿重复报名')" type="warning">
							报名课程
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card">
				<el-tab-pane label="课程大纲" name="first">
					<div class="ql-snow ql-editor" v-html="detail.kechengdagang"></div>
				</el-tab-pane>
		...
		methods: {
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$http.get('discusskechengxinxi/page',{params: {
							page: 1,
							limit: 1,
			...
			addDiscussNum(type){
				...
				this.$http.get('discusskechengxinxi/list',{params: {
					page: 1,
					limit: 10000,
					refid: this.detail.id
				}}).then(rs=>{
