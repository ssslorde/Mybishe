	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('zuoyexinxi','新增')" type="primary" @click="add('/index/zuoyexinxiAdd')">
			...
			<div class="list">
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)">
						<div class="imgbox">
							<img @click.stop="imgPreView(item.kechengfengmian.split(',')[0])" v-if="item.kechengfengmian && item.kechengfengmian.substr(0,4)=='http'" :src="item.kechengfengmian.split(',')[0]" class="image" />
						...
						<div class="infoBox">
							<div class="name">{{item.zuoyebianhao}}</div>
							<div class="name">{{item.kechengmingcheng}}</div>
							<div class="name">作业进度:{{item.zuoyejindu}}</div>
							<div class="bottomInfo">
								...
								<div class="publisher_item">
									...
									<span class="text">{{item.jiaoshizhanghao}}</span>
								</div>
							</div>
						</div>
					</div>
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/xueke/xueke').then(res => {
				if (res.data.code == 0) {
					this.xuekeOptions = res.data.data;
					this.selectOptionsList.push({name:'学科',list:this.xuekeOptions,tableName: 'xueke',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.xueke != '') searchWhere.xueke = this.formSearch.xueke;
				if (this.formSearch.xingming != '') searchWhere.xingming = '%' + this.formSearch.xingming + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`zuoyexinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/zuoyexinxiDetail', query: params});
			},
