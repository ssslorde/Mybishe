<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="作业编号" prop="zuoyebianhao">
				<el-input v-model="ruleForm.zuoyebianhao" placeholder="作业编号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="作业附件" prop="zuoyefujian">
				<file-upload
					tip="点击上传作业附件"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item"  label="作业进度" prop="zuoyejindu">
				<el-select v-model="ruleForm.zuoyejindu" placeholder="请选择作业进度" :disabled="true  ||ro.zuoyejindu" >
					<el-option
	...
	export default {
		...
		methods: {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zuoyexinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			zuoyefujianUploadChange(fileUrls) {
				this.ruleForm.zuoyefujian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
