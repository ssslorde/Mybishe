	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-else-if="ruleForm.zuoyebianhao" label="作业编号" prop="zuoyebianhao" >
					<el-input v-model="ruleForm.zuoyebianhao" placeholder="作业编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item v-else-if="ruleForm.zuoyefujian" label="作业附件" prop="zuoyefujian" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.zuoyefujian)">
				...
				<el-form-item class="select" v-if="type!='info'"  label="作业进度" prop="zuoyejindu" >
					<el-select :disabled="ro.zuoyejindu" v-model="ruleForm.zuoyejindu" placeholder="请选择作业进度" >
						<el-option
							v-for="(item,index) in zuoyejinduOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="作业进度" prop="zuoyejindu" >
					<el-input v-model="ruleForm.zuoyejindu"
						placeholder="作业进度" readonly></el-input>
				</el-form-item>
			</template>
...
<script>
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='zuoyejindu'){
							this.ruleForm.zuoyejindu = obj[o];
							this.ro.zuoyejindu = true;
							continue;
						}
					}
					this.ruleForm.zuoyejindu = '待完成'; 				}
				// 获取用户信息
				this.$http({
				...
				this.zuoyejinduOptions = "已完成,待完成".split(',')
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.zuoyebianhao) {
						this.ruleForm.zuoyebianhao = String(this.ruleForm.zuoyebianhao)
					}
